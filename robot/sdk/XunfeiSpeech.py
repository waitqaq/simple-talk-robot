...
def tts_on_open(ws):
    def run(*args):
        d = {"common": tts_wsParam.CommonArgs,
             "business": tts_wsParam.BusinessArgs,
             "data": tts_wsParam.Data,
             }
        d = json.dumps(d)
        ws.send(d)
...
# --------------------------------------------------------->
def transcribe(fpath, appid, apikey, apiSecret):
    global asr_wsParam, gResult
    # 科大讯飞ASR
    gResult = ""
    asr_wsParam = asr_Ws_Param(appid, apikey, apiSecret, AudioFile=fpath)
...
def synthesize(phrase, appid, apikey, apiSecret, voice_name="xioayan"):
    """
    讯飞tts
    """
    global tts_wsParam, gTTSResult, gTTSPath
    gTTSResult = " "
    with tempfile.NamedTemporaryFile() as f:
        gTTSPath = f.name
    tts_wsParam = tts_Ws_Param(APPID=appid, APIKey=apikey,
                               APISecret=apiSecret,
                               Text=phrase, voice_name=voice_name)
    websocket.enableTrace(False)
    wsUrl = tts_wsParam.create_url()
